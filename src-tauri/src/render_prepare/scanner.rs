...
pub struct ScopeState {
    pub text_lang_km: bool,
    pub par_justify: bool,
    pub text_hyphenate: bool,
}
...
fn parse_set_rule_args(args_str: &str, scope: &mut ScopeState, is_text: bool) {
    ...
    if is_text {
        if clean.contains("lang:\"km\"") || clean.contains("lang:\'km\'")
            || clean.contains("lang:\"khmer\"") || clean.contains("lang:\'khmer\'") {
            scope.text_lang_km = true;
        } else if clean.contains("lang:") {
            scope.text_lang_km = false;
        }
        
        if clean.contains("hyphenate:true") {
            scope.text_hyphenate = true;
        } else if clean.contains("hyphenate:false") {
            scope.text_hyphenate = false;
        }
    } else {
...
fn match_set_rule(chars: &[(usize, char)], start_idx: usize) -> Option<(bool, String)> {
    ...
    let is_text;
    if idx + 4 < chars.len() && chars[idx].1 == 't' && chars[idx + 1].1 == 'e' && chars[idx + 2].1 == 'x' && chars[idx + 3].1 == 't' {
        is_text = true;
        idx += 4;
    } else if idx + 3 < chars.len() && chars[idx].1 == 'p' && chars[idx + 1].1 == 'a' && chars[idx + 2].1 == 'r' {
        is_text = false;
        idx += 3;
    } else {
        return None;
...
pub fn scan_typst_content(content: &str) -> Vec<(ScanState, usize, usize, ScopeState)> {
    ...
    while i < chars.len() {
        ...
        if let Some((is_text, args_str)) = match_set_rule(&chars, check_idx) {
            if pos > chunk_start {
                chunks.push((current_state, chunk_start, pos, scopes.last().cloned().unwrap_or_default()));
            }
            chunk_start = pos;
        ...
        match current_state {
            ScanState::MarkupText => {
                if c == '[' {
                    if pos > chunk_start {
                        chunks.push((ScanState::MarkupText, chunk_start, pos, scopes.last().cloned().unwrap_or_default()));
                    }
            ...
            ScanState::LineComment => {
                if c == '\n' {
                    ...
                    chunks.push((ScanState::LineComment, chunk_start, end_pos, scopes.last().cloned().unwrap_or_default()));
            ...
            ScanState::BlockComment => {
                if c == '*' && i + 1 < chars.len() && chars[i + 1].1 == '/' {
                    ...
                    chunks.push((ScanState::BlockComment, chunk_start, end_pos, scopes.last().cloned().unwrap_or_default()));
                    current_state = ScanState::MarkupText;
                    chunk_start = end_pos;
                    i += 2;
                    continue;
            ...
            ScanState::RawBlock => {
                if c == '`' && i + 2 < chars.len() && chars[i + 1].1 == '`' && chars[i + 2].1 == '`' {
                    ...
                    chunks.push((ScanState::RawBlock, chunk_start, end_pos, scopes.last().cloned().unwrap_or_default()));
                    current_state = ScanState::MarkupText;
                    chunk_start = end_pos;
                    i += 3;
                    continue;
            ...
            ScanState::RawInline => {
                if c == '`' {
                    let end_pos = pos + 1;
                    chunks.push((ScanState::RawInline, chunk_start, end_pos, scopes.last().cloned().unwrap_or_default()));
                    current_state = ScanState::MarkupText;
                    chunk_start = end_pos;
                    i += 1;
                    continue;
            ...
            ScanState::Math => {
                if c == '$' {
                    let end_pos = pos + 1;
                    chunks.push((ScanState::Math, chunk_start, end_pos, scopes.last().cloned().unwrap_or_default()));
                    current_state = ScanState::MarkupText;
                    chunk_start = end_pos;
                    i += 1;
                    continue;
            ...
            ScanState::CodeExpression => {
                ...
                if c == '(' {
                    ...
                    if pos > chunk_start {
                        chunks.push((ScanState::CodeExpression, chunk_start, pos, scopes.last().cloned().unwrap_or_default()));
                    }
                    bracket_stack.push(']');
                    scopes.push(scopes.last().cloned().unwrap_or_default());
                    current_state = ScanState::MarkupText;
    ...
    if end_pos > chunk_start {
        chunks.push((current_state, chunk_start, end_pos, scopes.last().cloned().unwrap_or_default()));
    }
...
mod tests {
    ...
    fn test_scan_plain_markup() {
        ...
        assert_eq!(chunks[0], (ScanState::MarkupText, 0, content.len(), ScopeState::default()));
    }
    ...
    fn test_scan_scope_aware_set_rules() {
        ...
        for chunk in &chunks {
            ...
            if chunk_text.contains("Text 1") {
                assert!(chunk.3.text_lang_km);
                assert!(chunk.3.par_justify);
                assert!(!chunk.3.text_hyphenate);
                text_1_found = true;
