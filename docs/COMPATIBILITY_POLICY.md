...
This document defines versioning rules, stability contracts, and promotion criteria for Typstry's language provider and editing policy systems.
...
If an installed language entry refers to an unknown provider ID, Typstry must silently remove the entry rather than error. The provider can be re-installed by the user.
