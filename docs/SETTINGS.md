...
Document Typography assigns one font and uniform scale to each configured script. Typsastra emits native Typst `covers` descriptors using Unicode Script Extensions (`scx`), so a font's extra glyphs cannot consume text assigned to another script and the entries may appear in any order. Values other than `1.0` generate a render-only font under `.typsastra/fonts/generated` and restart Tinymist with that directory as a project font path. Non-unit scaling is experimental for PDF output because Typst may normalize generated fonts while subsetting them; use `1.0` when dependable PDF export is required. Typsastra does not create script-matching show rules because they break character-level inverse sync, and it does not patch the resulting PDF or make preview differ from export. Raw code keeps Typst's original raw font. See [Document typography](DOCUMENT_TYPOGRAPHY.md).
